...
function shoppingTime(memberId, money) {
  ...
  const list_produk = ['Sepatu Stacattu','Baju Zoro','Baju H&N','Sweater Uniklooh','Casing Handphone'];
  const list_harga = [1500000, 500000, 250000, 175000, 50000];
  ...
  if (memberId === "") {
    ...
    for (let i = 0; i < list_produk.length; i++) {
      if (money >= list_harga[i]) {
        dibeli.push(list_produk[i]);
        uang -= list_harga[i];
      }
    }
