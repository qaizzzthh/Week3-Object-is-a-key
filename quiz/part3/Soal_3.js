...
function countProfit(shoppers) {
  ...
  for (let i = 0; i < listBarang.length; i++) {
    ...
    let pembeli = "";
    let dibeli = 0;
    let total_kebutuhan = 0;
    for (let j = 0; j < shoppers.length; j++) {
      if (shoppers[j].product === listBarang[i][0]) {
        pembeli = shoppers[j].name;
        belanja.push(pembeli);
      }

      if (shoppers[j].product === listBarang[i][0]) {
        dibeli = shoppers[j].amount;
        total_kebutuhan += dibeli;
      }
    }

    if (listBarang[i][2] < total_kebutuhan) {
      total_kebutuhan = 0;
      belanja = [];
    }
    ...
    keterangan = {
      ...
      shoppers: belanja,
      leftOver: listBarang[i][2] - total_kebutuhan,
      totalProfit: listBarang[i][1] * total_kebutuhan,
    }
      hasil.push(keterangan);
  }
